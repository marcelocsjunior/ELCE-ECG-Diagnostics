<#
.SYNOPSIS
    Core unificado de diagnóstico e correções BDE/ECG.

.DESCRIPTION
    Um único ponto de entrada para:
      - DIAG          -> executa o diagnóstico oficial e gera o laudo HTML
      - NETDIR        -> corrige a entrada NETDIR no HKLM
      - HW_CAMINHO_DB -> corrige a variável de ambiente
      - DIRECTORIES   -> cria C:\HW\NetDir e C:\HW\Private
      - ALL           -> aplica NETDIR + HW_CAMINHO_DB + DIRECTORIES

    Compatível com Windows PowerShell 5.1.

.PARAMETER Profile
    UN1, UN2, UN3 ou CUSTOM.

.PARAMETER CustomDbPath
    Caminho do banco quando Profile=CUSTOM.

.PARAMETER CustomNetDir
    Caminho do NetDir quando Profile=CUSTOM. Se omitido, deriva de CustomDbPath\NetDir.

.PARAMETER TaskMode
    DIAG, NETDIR, HW_CAMINHO_DB, DIRECTORIES ou ALL.

.PARAMETER HwScope
    Escopo da variável HW_CAMINHO_DB: User, Machine ou Process.

.PARAMETER ProfilesFile
    Caminho opcional do ECG_UnitProfiles.json.

.PARAMETER DiagnosticScript
    Caminho opcional do ELCE_ECG_Diagnostics.ps1.

.PARAMETER OutputRoot
    Diretório de saída dos relatórios TXT/JSON do core.
#>
